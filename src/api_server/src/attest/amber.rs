...
impl Attest for Amber {
    async fn attest_verify(
        ...
        if tee != Tee::Tdx {
            return Err(anyhow!("Only implement for tdx now"));
        }
        // get quote
        ...
        // find jwk
        let key = self.jwks.find(&kid).ok_or(anyhow!("Find jwk failed"))?;
        ...
        // check policy
        let check_policy = self.config.check_policy.unwrap_or(true);
        if check_policy && token.claims.amber_unmatched_policy_ids.is_some() {
            return Err(anyhow!("Evidence doesn't match policy"));
        }
...
impl Amber {
    pub fn new(amber_config: &Option<AmberConfig>) -> Result<Self> {
        ...
        Ok(Self {
            config: config,
            jwks: serde_json::from_reader(reader)
                .map_err(|e| anyhow!("Deserialize jwks failed: {:?}", e))?,
        })
    }
}
